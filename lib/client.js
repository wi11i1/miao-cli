...
async function render(name, path, params, cfg) {
  ...
  const { filepath, removeHtml } = await writeHtml(name, html);
...
async function writeHtml(name, html) {
  ...
  const htmlDir = `${process.cwd()}/data/html/${uuid}/`;
  fs.existsSync(htmlDir) || (await fs.promises.mkdir(htmlDir, { recursive: true }));

  const filepath = htmlDir + `/${name}.html`;
  await fs.promises.writeFile(filepath, html);

  const removeHtml = () => fs.promises.rm(htmlDir, { recursive: true, force: true });

  return { filepath, removeHtml };
}
