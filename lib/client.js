...
import { Character, Player } from "../plugins/miao-plugin/models/index.js";
import ProfileList from "../plugins/miao-plugin/apps/profile/ProfileList.js";

import "./global.js";

const reply = (...args) => logger.info(...args);

export async function getPlayerAvatars(uid) {
  let player = Player.create({ uid, reply });
  if (Object.keys(player._avatars).length === 0) {
    await player.refreshProfile(2);
  }
  return Object.entries(player._avatars).map(([id, avatar]) => {
    return { value: id, name: avatar.char.name };
  });
}

export async function createImge({ uid, avatar, mode }) {
  const e = {
    user_id: uid,
    uid,
    avatar,
